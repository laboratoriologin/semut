...
import com.login.android.semut.lauro.adapter.ListItemAdapterMeusRegistros;
...
public class MeusRegistrosActivity extends DefaultActivity implements OnTouchListener, OnItemClickListener {
	...
	public class MyRequestReceiver extends BroadcastReceiver {
		...
		public static final String PROCESS_RESPONSE = "com.login.android.semut.lauro.intent.action.PROCESS_RESPONSE";

		@Override
		public void onReceive(Context context, Intent intent) {

			Ocorrencia objOcorrenciaTrans = (Ocorrencia) intent.getSerializableExtra(Constantes.PARAM_OCORRENCIA);

			for (Object item : lstItemAdapterOcorrencia.getResultado()) {
				if (item instanceof Ocorrencia && objOcorrenciaTrans.getId().equals(((Ocorrencia) item).getId())) {
					((Ocorrencia) item).setNumeroProtocolo(objOcorrenciaTrans.getNumeroProtocolo());
					break;
				}
			}
	...
	public void onBackPressed() {
		...
		Intent intent;
		intent = new Intent(this, TransalvadorActivity.class);
