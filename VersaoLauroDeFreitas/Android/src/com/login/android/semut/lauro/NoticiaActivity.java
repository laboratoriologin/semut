...
public class NoticiaActivity extends DefaultActivity implements OnTouchListener, OnItemClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		idFlag = (Integer) getIntent().getSerializableExtra(ID_FLAG);
		...
		if (idFlag == Constantes.Flags.TRANSALVADOR) {
			...
			if (Utilitarios.hasConnection(this)) {
				...
				progressDialog.setMessage(Constantes.BUSCANDO_NOTICIAS);
			...
			if (Utilitarios.hasConnection(this)) {
				...
				progressDialog.setMessage(Constantes.BUSCANDO_AVISOS);
	...
	public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
		...
		intent.putExtra(Constantes.NOTICIA, noticia);

		if (Constantes.CAT_TRANSALVADOR.equals(grupo.getId())) {

			intent.putExtra(Constantes.ID_FLAG, Constantes.Flags.NOTICIA);

		} else {

			intent.putExtra(Constantes.ID_FLAG, Constantes.Flags.AVISO);
